...
export default function ChatScreen({ route, navigation }: Props) {
    const { patientId, doctorId, patientName, profilePicUrl } = route.params;
    ...
    useEffect(() => {
        ...
        socket.on('receive_message', (data: any) => {
            ...
            const isOurMessage = data.sender === 'PATIENT';
            ...
            <View className="flex-row items-center px-4 py-4 border-b border-gray-100 bg-white">
                ...
                <View className="flex-1">
                    <Text className="text-gray-900 font-bold text-lg">
                        {patientName ? `Dr. ${patientName}` : 'Doctor'}
                    </Text>
                    <Text className="text-gray-500 text-sm">Doctor Chat</Text>
                </View>
            </View>
